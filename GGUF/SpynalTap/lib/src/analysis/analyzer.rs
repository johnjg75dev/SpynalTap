...
impl Analyzer {
    ...
    pub fn with_sample_per_tensor(n: usize) -> Self {
        Self { sample_per_tensor: n, keep: HashSet::new() }
    }
    ...
    pub fn sample_per_tensor(&self) -> usize { self.sample_per_tensor }
    ...
    pub fn analyze<M: Model + ?Sized>(&self, model: &M) -> Result<Analysis> {
        ...
        let mut per_tensor: Vec<(crate::model::Tensor, TensorStats)> = Vec::with_capacity(tensors.len());
...
fn dequant_for_dtype(dtype: TensorDtype, bytes: &[u8], max_elems: Option<usize>) -> Option<Vec<f32>> {
...
fn recommend(blocks: &[BlockAnalysis], total_bytes: u64) -> (Vec<i32>, usize, u64) {
    let mut prunable: Vec<&BlockAnalysis> = blocks.iter().filter(|b| b.role.is_prunable()).collect();
    prunable.sort_by(|a, b| b.removable.partial_cmp(&a.removable).unwrap_or(std::cmp::Ordering::Equal));
