...
pub fn quantize(src: &[f32]) -> Vec<u8> {
    ...
    for blk in src.chunks_exact(QK_K) {
        ...
        let d = if block_max == 0.0 { 0.0 } else { block_max / 15.0 };
        let dmin = if block_min >= 0.0 { 0.0 } else { -block_min / 15.0 };
        ...
        for s in 0..N_SUB {
            ...
            sc[s] = if range > 0.0 { sc_r.clamp(1.0, 63.0) as u8 } else { 0 };
        ...
        for s in 4..N_SUB {
            if sc[s] == 0 && mn[s] == 0 { continue; }
            let lo_sc = sc[s] & 0x0F;
            let lo_mn = mn[s] & 0x0F;
            if lo_sc == lo_mn { continue; }
            let sub = &blk[s * SUB..(s + 1) * SUB];
