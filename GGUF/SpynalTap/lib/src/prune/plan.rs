...
pub fn build_plan<M: Model + ?Sized>(model: &M, sel: &Selection, scores: Option<&[BlockAnalysis]>) -> Result<PrunePlan> {
    ...
    let drop: HashSet<i32> = match sel {
        ...
        Selection::Keep(keeps) => sorted_blocks.iter().filter(|i| !keeps.contains(i)).cloned().collect(),
        Selection::Drop(d) => d.iter().cloned().collect(),
        Selection::Auto(n) => {
            let n = (*n).min(sorted_blocks.len());
            let scores = scores.ok_or_else(|| crate::Error::InvalidSelection(
                "auto".into(), "requires analysis scores".into(),
            ))?;
            let mut ranked: Vec<(i32, f64)> = sorted_blocks.iter().filter_map(|i| {
                scores.iter().find(|b| b.index == *i).map(|b| (*i, b.removable))
            }).collect();
        ...
        Selection::Pattern(re) => {
            ...
            for t in model.tensors() {
                if re.is_match(&t.name) {
                    let (role, idx, _) = classify(&t.name);
                    if role == BlockRole::Block { hits.insert(idx); }
                }
            }
            hits
        }
    ...
    let mut kept_sorted: Vec<i32> = sorted_blocks.iter().filter(|i| !drop.contains(i)).cloned().collect();
    kept_sorted.sort();
    ...
    Ok(PrunePlan { keep, remap, dropped_blocks, original_block_count, new_block_count })
}
