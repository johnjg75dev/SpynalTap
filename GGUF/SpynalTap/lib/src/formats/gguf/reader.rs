...
impl GgufFile {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let mmap = unsafe { Mmap::map(&file) }
            .map_err(|e| Error::Gguf(format!("mmap failed: {e}")))?;
    ...
    pub fn metadata_str(&self, key: &str) -> Option<&str> {
        self.metadata.iter().find(|k| k.key == key).and_then(|kv| {
            if let MetaValue::String(s) = &kv.value { Some(s.as_str()) } else { None }
        })
    }

    pub fn metadata_u32(&self, key: &str) -> Option<u32> {
        self.metadata.iter().find(|k| k.key == key).and_then(|kv| match &kv.value {
...
impl Model for GgufFile {
    ...
    fn name(&self) -> Option<&str> { self.metadata_str("general.name") }
    fn architecture(&self) -> Option<&str> { self.metadata_str("general.architecture") }
    fn block_count(&self) -> Option<usize> {
        let arch = self.architecture()?;
        self.metadata_u32(&format!("{arch}.block_count")).map(|v| v as usize)
    }
    fn tensors(&self) -> &[Tensor] { &self.model_tensors }
    ...
    fn read_tensor_bytes(&self, name: &str) -> Result<Cow<'_, [u8]>> {
        ...
        match self.tensor_slice(t) {
            ...
            None => Err(Error::Gguf(
                "tensor bytes unavailable: file was parsed from a stream, not opened with mmap".into(),
            )),
        }
    }
}
...
trait ReadExt: Read {
    ...
    #[inline]
    fn read_i16_le(&mut self) -> std::io::Result<i16> { Ok(self.read_u16_le()? as i16) }
    #[inline]
    ...
    #[inline]
    fn read_i32_le(&mut self) -> std::io::Result<i32> { Ok(self.read_u32_le()? as i32) }
    #[inline]
    ...
    #[inline]
    fn read_i64_le(&mut self) -> std::io::Result<i64> { Ok(self.read_u64_le()? as i64) }
    #[inline]
...
fn read_kv<R: Read>(r: &mut R) -> Result<MetadataKv> {
    ...
    Ok(MetadataKv { key, value_type, value })
}
...
fn read_meta_value<R: Read>(r: &mut R, ty: u32) -> std::io::Result<MetaValue> {
    Ok(match ty {
        ...
        9 => {
            ...
            MetaValue::Array(ArrayValue { elem_type, elements: elems })
        }
...
fn read_tensor_info<R: Read>(r: &mut R) -> Result<TensorInfo> {
    ...
    if n_dims > 4 {
        return Err(Error::Gguf(format!("tensor '{name}' has {n_dims} dims (max 4)")));
    }
