...
pub fn verify(path: &Path, expected_names: &[String]) -> Result<VerifyReport> {
    ...
    for w in ranges.windows(2) {
        ...
        let (s2, _, n2) = w[1];
        if s2 < e1 {
            report.errors.push(format!("tensor overlap: {s1}..{e1} collides with '{n2}' at {s2}"));
        }
    }
    if let Some((_, e, last)) = ranges.last() {
        if *e > file_size {
            report.errors.push(format!("last tensor '{last}' ends at {e} past file size {file_size}"));
        }
    }
    ...
    if gguf.tensors.is_empty() {
        ...
        report.warnings.push("all tensors report zero byte size".into());
    }
