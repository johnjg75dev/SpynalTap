...
use crate::formats::gguf::types::{byte_size_for, dims_product, GgmlType, MetaValue, MetadataKv, GGUF_MAGIC};
...
impl GgufWriter {
    ...
    #[inline]
    pub fn add_kv(&mut self, kv: MetadataKv) { self.metadata.push(kv); }
    ...
    pub fn add_tensor(&mut self, name: String, n_dims: u32, dims: [u64; 4], ty: GgmlType, bytes: &[u8]) {
        ...
        self.tensors.push(WriterTensor {
            name, n_dims, dims, ggml_type: ty, offset, n_elements, byte_size,
        });
...
fn push_meta_value(out: &mut Vec<u8>, v: &MetaValue) {
    match v {
        ...
        MetaValue::Array(a) => {
            ...
            for e in &a.elements { push_meta_value(out, e); }
        }
    }
}
...
fn meta_value_size_estimate(v: &MetaValue) -> usize {
    match v {
        ...
        MetaValue::Array(a) => 12 + a.elements.iter().map(meta_value_size_estimate).sum::<usize>(),
        _ => 8,
    }
}
