...
impl SafetensorsFile {
    ...
    pub fn from_mmap(mmap: Mmap) -> Result<Self> {
        ...
        for (name, e) in header.entries {
            let dtype = dtype_from_str(&e.dtype)
                .ok_or_else(|| Error::Safetensors(format!("unsupported dtype {dtype:?}", dtype = e.dtype)))?;
...
impl Model for SafetensorsFile {
    fn format(&self) -> ModelFormat { ModelFormat::Safetensors }
    ...
    fn metadata(&self, _key: &str) -> Option<MetadataValue<'_>> { None }
    fn read_tensor_bytes(&self, name: &str) -> Result<Cow<'_, [u8]>> {
        let t = self.name_to_idx.get(name)
...
fn dtype_from_str(s: &str) -> Option<TensorDtype> {
    Some(match s {
        "F32" => TensorDtype::F32, "F16" => TensorDtype::F16, "BF16" => TensorDtype::Bf16, "F64" => TensorDtype::F64,
        "I8" => TensorDtype::I8, "I16" => TensorDtype::I16, "I32" => TensorDtype::I32, "I64" => TensorDtype::I64,
