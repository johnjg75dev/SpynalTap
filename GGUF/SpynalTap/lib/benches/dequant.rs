...
fn bench_dequant_f16(c: &mut Criterion) {
    let bytes: Vec<u8> = (0..(1 << 20)).map(|i| ((i * 7 + 3) & 0xFFFF) as u16 as u8).cycle().take(1 << 21).collect();
...
criterion_group!(benches, bench_dequant_f16, bench_dequant_q8_0, bench_dequant_q4_0);
