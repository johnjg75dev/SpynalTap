...
use spynaltap::svd::config::{LayerSelection, OutputDtype, RankClamps, RankSpec, RankSpecWithClamps, SvdConfig, TensorSelection};
...
fn parse_layer_selection_aliases() {
    assert!(matches!(LayerSelection::parse("all").unwrap(), LayerSelection::All));
    assert!(matches!(LayerSelection::parse("all-attn").unwrap(), LayerSelection::AllAttn));
    assert!(matches!(LayerSelection::parse("all-ffn").unwrap(), LayerSelection::AllFfn));
    ...
    assert!(matches!(LayerSelection::parse("regex:^blk\\.0\\.").unwrap(), LayerSelection::Pattern(_)));
...
fn rel_err(a: &[f32], b: &[f32]) -> f32 {
    let mut n = 0.0f64; let mut d = 0.0f64;
    for i in 0..a.len() {
        let diff = a[i] as f64 - b[i] as f64;
        n += diff * diff;
        d += (a[i] as f64) * (a[i] as f64);
    }
    if d == 0.0 { 0.0 } else { (n / d).sqrt() as f32 }
}
...
fn svd_jacobi_diagonal_recovers() {
    ...
    for i in 0..n { data[i * n + i] = (n - i) as f32; }
    ...
    for i in 0..n { assert!((got[i] - (n - i) as f32).abs() < 1e-3, "got[{i}] = {}", got[i]); }
}

#[test]
fn svd_jacobi_low_rank_reconstruction() {
    // Build a clean rank-1 matrix: A = u * v^T. Singular value = ||u|| * ||v||.
    let m = 4usize; let n = 3usize; // non-square, m != n
    ...
    for i in 0..m { for j in 0..n { a_data[i * n + j] = u[i] * v[j]; } }
    let expected_s0: f32 = {
    ...
    assert!((svd.s[0] - expected_s0).abs() < 1e-2, "s[0] = {} (want {})", svd.s[0], expected_s0);
    ...
    let a1_data: Vec<f32> = (0..a_pack.rows).map(|r| a_pack.data[r * k_full_a]).collect();
    let a1 = Mat { rows: a_pack.rows, cols: 1, data: a1_data };
    // b_pack is k_full x n; take the first k rows.
    let b1 = Mat { rows: 1, cols: b_pack.cols, data: b_pack.data[..b_pack.cols].to_vec() };
...
fn svd_randomized_matches_jacobi() {
    ...
    for i in 0..m { for j in 0..n { a_data[i * n + j] = u[i] * v[j] * 0.5 + u2[i] * v2[j] * 0.3; } }
    ...
    assert!((s_rand.s[0] - s_jac.s[0]).abs() / s_jac.s[0].max(1e-6) < 0.05,
            "rand s[0] = {} vs jacobi {}", s_rand.s[0], s_jac.s[0]);
}
...
fn tiny_gguf_with_attn_q(path: &std::path::Path) {
    ...
    w.add_kv(MetadataKv { key: "llama.block_count".into(), value_type: 4, value: MetaValue::U32(2) });
    // 8x4 weight matrix (rank-1 truth: A = u * v^T scaled by 3.0).
    let m = 8usize; let n = 4usize;
    ...
    for i in 0..m { for j in 0..n { w_data[i * n + j] = 3.0 * u_true[i] * v_true[j]; } }
    let mut bytes = Vec::with_capacity(m * n * 4);
    for v in &w_data { bytes.extend_from_slice(&v.to_le_bytes()); }
    ...
    w.add_tensor("blk.0.attn_norm.weight".into(), 1, [8, 1, 1, 1], GgmlType::F32, &extra);
    // 1D embd-like (16 f32 = 64 bytes).
    let embd: Vec<u8> = (0..16u32).flat_map(|i| ((i as f32) * 0.5).to_le_bytes()).collect();
    w.add_tensor("token_embd.weight".into(), 1, [16, 1, 1, 1], GgmlType::F32, &embd);
...
fn apply_to_gguf_writes_expected_structure() {
    ...
    assert_eq!(a.m, 8); assert_eq!(a.n, 4); assert_eq!(a.k, 2);
    assert!(a.approx_error < 0.05, "approx_error too high: {}", a.approx_error);
    ...
    assert!(!names.contains(&"blk.0.attn_q.weight"), "original tensor should be removed");
    ...
    // Metadata checks.
    assert_eq!(out_gg.metadata_str("spynaltap.svd.applied").map(|s| s == "true").unwrap_or(false)
               || out_gg.metadata_u32("spynaltap.svd.targets") == Some(1), true);
...
fn apply_to_gguf_preserves_non_targets() {
    ...
    cfg.layers = LayerSelection::AllMlp;
    cfg.tensors = TensorSelection::Attn;
    cfg.rank = RankSpecWithClamps { spec: RankSpec::Absolute(2), clamps: RankClamps { min: 1, max: None } };
    ...
    let src_bytes = gg.read_tensor_bytes("blk.0.attn_norm.weight").unwrap().to_vec();
    let dst_bytes = out_gg.read_tensor_bytes("blk.0.attn_norm.weight").unwrap().to_vec();
