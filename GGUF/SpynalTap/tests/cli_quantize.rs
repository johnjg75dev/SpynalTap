...
fn cli_quantize_q4_0_smoke() {
    let in_path = std::env::temp_dir().join(format!("spynaltap-cli-in-{}.gguf", std::process::id()));
    let out_path = std::env::temp_dir().join(format!("spynaltap-cli-out-{}.gguf", std::process::id()));
    ...
    assert!(stdout.contains("\"tensors_quantized\": 1"), "stdout: {stdout}");
    ...
    let ti = gg.tensors.iter().find(|t| t.name == "blk.0.attn_q.weight").unwrap();
