...
pub fn apply_to_gguf(gg: &GgufFile, plan: &SvdPlan, dst: &Path) -> Result<SvdReport> {
    ...
    for kv in &gg.metadata { writer.add_kv(kv.clone()); }
    for kv in build_svd_kvs(plan) { writer.add_kv(kv); }
    ...
    for ti in &gg.tensors {
        if let Some(target) = targets.get(ti.name.as_str()) {
            ...
            let deq = dequant_to_f32(ti.ggml_type, bytes)
                .ok_or_else(|| Error::Svd(format!("unsupported dtype for SVD: {}", ti.ggml_type.as_str())))?;
            if deq.len() != target.m * target.n {
                return Err(Error::Svd(format!(
                    "tensor '{}': dequantized {} elems, expected {}",
                    ti.name, deq.len(), target.m * target.n
                )));
            }
            ...
            writer.tensors.push(WriterTensor {
                name: target.name_a.clone(), n_dims: 2,
                dims: [target.m as u64, effective_k as u64, 1, 1],
                ggml_type: a_ty, offset: a_offset, n_elements: a_n, byte_size: a_bz,
            });
            ...
            writer.tensors.push(WriterTensor {
                name: target.name_b.clone(), n_dims: 2,
                dims: [effective_k as u64, target.n as u64, 1, 1],
                ggml_type: a_ty, offset: b_offset, n_elements: b_n, byte_size: b_bz,
            });
            ...
            { "randomized".to_string() } else { "jacobi".to_string() };
            applied.push(SvdApplied {
                name: ti.name.clone(),
                name_a: target.name_a.clone(),
                name_b: target.name_b.clone(),
                m: target.m, n: target.n, k: effective_k,
                method,
    ...
    Ok(SvdReport {
        ...
        applied,
        skipped: plan.skipped.iter().map(|s| (s.name.clone(), s.reason.clone())).collect(),
        output_path: dst.display().to_string(),
        orig_tensor_bytes: total_orig,
        new_tensor_bytes: total_new,
        compression_ratio: if total_orig == 0 { 0.0 } else { 1.0 - (total_new as f64 / total_orig as f64) },
    })
}
...
pub fn apply_to_safetensors(st: &SafetensorsFile, plan: &SvdPlan, dst: &Path) -> Result<SvdReport> {
    ...
    for t in &st.tensors {
        if let Some(target) = targets.get(t.name.as_str()) {
            ...
            let f32_vals = match t.dtype {
                ...
                other => return Err(Error::Svd(format!("unsupported safetensors dtype for SVD: {:?}", other))),
            };
            if f32_vals.len() != target.m * target.n {
                return Err(Error::Svd(format!("tensor '{}': decoded {} elems, expected {}", t.name, f32_vals.len(), target.m * target.n)));
            }
            let a_mat = Mat::from_vec(target.m, target.n, f32_vals);
            let k = if target.k == 0 {
                plan.config.resolve_rank(&t.name, 0, target.m, target.n, None)
            } else { target.k };
            ...
            writer.add_raw(target.name_a.clone(), dtype_to_tensordtype(dt), vec![target.m as u64, effective_k as u64], &a_bytes);
            writer.add_raw(target.name_b.clone(), dtype_to_tensordtype(dt), vec![effective_k as u64, target.n as u64], &b_bytes);
            ...
            applied.push(SvdApplied {
                ...
                name_b: target.name_b.clone(),
                m: target.m, n: target.n, k: effective_k,
                method, orig_bytes: target.orig_bytes, new_bytes, approx_error: approx_err,
            });
        } else {
            let bytes = st.read_tensor_bytes(&t.name)?;
    ...
    header_meta.insert("output_dtype".into(), serde_json::json!(plan.config.dtype.as_str()));
...
fn run_svd(a_mat: &Mat, requested_k: usize, cfg: &crate::svd::config::SvdConfig,
           name: &str, target: &SvdTarget) -> Result<(crate::svd::linalg::Svd, usize)> {
    ...
    match &cfg.rank.spec {
        RankSpec::Energy(_) => {
            ...
            let svd_full = if use_randomized {
                svd_randomized(a_mat, full_k, cfg.randomized_oversample, cfg.randomized_power_iters, hash_seed(name, target))?
            } else {
                svd_jacobi(a_mat, 100, 1e-10)?
            };
            let k = cfg.resolve_rank(name, crate::analysis::score::classify(name).1,
                                     a_mat.rows, a_mat.cols, Some(&svd_full.s));
            Ok((svd_full, k))
        }
        _ => {
            let k = requested_k.max(1).min(a_mat.rows).min(a_mat.cols);
            let svd = if use_randomized {
                svd_randomized(a_mat, k, cfg.randomized_oversample, cfg.randomized_power_iters, hash_seed(name, target))?
            } else {
                svd_jacobi(a_mat, 100, 1e-10)?
            };
            Ok((svd, k))
...
fn encode_factors(m: &Mat, dtype: OutputDtype, rows: usize, cols: usize) -> Vec<u8> {
    ...
    match dtype {
        OutputDtype::F32 => {
            ...
            for &v in &m.data { out.extend_from_slice(&v.to_le_bytes()); }
            out
        }
        OutputDtype::F16 => {
            let mut out = Vec::with_capacity(rows * cols * 2);
            for &v in &m.data { out.extend_from_slice(&f32_to_f16_bits(v).to_le_bytes()); }
            out
        }
        OutputDtype::Bf16 => {
            let mut out = Vec::with_capacity(rows * cols * 2);
            for &v in &m.data { out.extend_from_slice(&f32_to_bf16_bits(v).to_le_bytes()); }
            out
        }
    }
}
...
fn f32_to_f16_bits(v: f32) -> u16 {
    ...
    if v.is_infinite() { return sign | 0x7c00; }
...
fn approx_error(original: &Mat, a: &Mat, b: &Mat) -> f32 {
    if original.data.is_empty() { return 0.0; }
    ...
    if den == 0.0 { 0.0 } else { (num / den).sqrt() as f32 }
}

fn hash_seed(name: &str, target: &SvdTarget) -> u64 {
    let mut h: u64 = 0xcbf2_9ce4_8422_2325;
    for b in name.as_bytes() { h ^= *b as u64; h = h.wrapping_mul(0x100_0000_01b3); }
    for &x in &[target.m, target.n, target.k] {
        for b in x.to_le_bytes() { h ^= b as u64; h = h.wrapping_mul(0x100_0000_01b3); }
    }
    h
}
...
fn build_svd_kvs(plan: &SvdPlan) -> Vec<MetadataKv> {
    ...
    kvs.push(MetadataKv { key: "spynaltap.svd.applied".into(), value_type: 7, value: MetaValue::Bool(true) });
    kvs.push(MetadataKv { key: "spynaltap.svd.method".into(), value_type: 8, value: MetaValue::String(method_str.into()) });
    kvs.push(MetadataKv { key: "spynaltap.svd.output_dtype".into(), value_type: 8, value: MetaValue::String(plan.config.dtype.as_str().into()) });
    kvs.push(MetadataKv { key: "spynaltap.svd.targets".into(), value_type: 4, value: MetaValue::U32(plan.targets.len() as u32) });
    let total_orig: u64 = plan.targets.iter().map(|t| t.orig_bytes).sum();
    let total_new: u64 = plan.targets.iter().map(|t| t.new_bytes).sum();
    kvs.push(MetadataKv { key: "spynaltap.svd.orig_bytes".into(), value_type: 10, value: MetaValue::U64(total_orig) });
    kvs.push(MetadataKv { key: "spynaltap.svd.new_bytes".into(), value_type: 10, value: MetaValue::U64(total_new) });
    let ratio = if total_orig == 0 { 0.0 } else { 1.0 - (total_new as f64 / total_orig as f64) };
    kvs.push(MetadataKv { key: "spynaltap.svd.compression_ratio".into(), value_type: 6, value: MetaValue::F32(ratio as f32) });

    for t in &plan.targets {
        let key = format!("spynaltap.svd.{}.rank", t.name);
        kvs.push(MetadataKv { key, value_type: 4, value: MetaValue::U32(t.k as u32) });
        ...
        kvs.push(MetadataKv {
            key, value_type: 9,
            value: MetaValue::Array(ArrayValue { elem_type: 6, elements: elems }),
        });
    }
    kvs
}
...
fn dt_str(d: TensorDtype) -> &'static str {
    match d {
        TensorDtype::F32 => "F32", TensorDtype::F16 => "F16", TensorDtype::Bf16 => "BF16",
        TensorDtype::F64 => "F64", TensorDtype::I8 => "I8", TensorDtype::I16 => "I16",
        TensorDtype::I32 => "I32", TensorDtype::I64 => "I64",
        _ => "U8",
    }
}
...
#[allow(dead_code)]
fn _silence(_: u64, _: u32) -> u64 { dims_product(&[1, 1, 1, 1], 1) }
