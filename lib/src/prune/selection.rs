...
pub fn parse_index_list(s: &str) -> Result<Vec<i32>> {
    ...
    for part in s.split(',') {
        ...
        if let Some((a, b)) = part.split_once('-') {
            let a: i32 = a.parse().map_err(|e| {
                Error::InvalidSelection(s.into(), format!("bad range '{part}': {e}"))
            })?;
            let b: i32 = b.parse().map_err(|e| {
                Error::InvalidSelection(s.into(), format!("bad range '{part}': {e}"))
            })?;
            ...
            let i: i32 = part.parse().map_err(|e| {
                Error::InvalidSelection(s.into(), format!("bad index '{part}': {e}"))
            })?;
            out.insert(i);
        }
    }
